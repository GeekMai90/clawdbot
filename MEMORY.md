...
1. **Cron 投递必须用 chatId，不能用用户名** 🗄️ 可归档（已被上方置顶核心第 6 条覆盖）
...
## 2026-03-15 每周蒸馏（近 7 天）

1. **Roam Daily Note 已形成明确的整理版式**
   麦先生本周确认了 Daily Note 的长期结构：按「时间日志 / 任务 / 其他」三段排序，条目使用不缩进的平铺格式，并保留“今日最佳”作为整理后的总结段名。后续凡是 Roam Daily Note 整理、重排或技能提示词设计，都应默认遵守这套版式，不要再临时猜格式。

2. **Codex 相关模型的认证本质上依赖 OAuth，需视为需维护资源**
   本周再次验证：`openai-codex/gpt-5.x` 这类 Codex 模型不是 API Key 路线，而是 OAuth 登录态；一旦 refresh 失效，默认模型链会整段报错。对应策略不是“找永久 Key”，而是保留每周健康检查与人工重新登录流程，把 Codex OAuth 当成需要定期维护的运行依赖。

3. **Codex 授权与 OpenClaw 内置浏览器/配置链路存在真实耦合**
   重新登录 Codex 时，授权页实际会在 OpenClaw 控制的独立浏览器中打开，而不是用户平时的 Chrome；排障时必须先确认用户是否在那个窗口完成了登录操作。相关配置上，错误 provider 或失效 token 会直接导致默认模型切换失败，因此“先重登 OAuth，再动默认模型”应作为优先顺序。

4. **定时任务若绑定到缺工具能力的运行环境，会出现“会说不会做”的假失败** 🗄️ 可归档（已被 2026-03-22 第 4 条复述并细化）
   本周出现过 cron 能触发、但执行环境缺少 `exec/read` 等工具，结果只能回报“当前运行环境未提供可执行工具”的情况。后续排查 cron/skill 失败时，除了看脚本本身，也要先检查任务绑定的 agent/session/tool 能力是否覆盖所需执行链路。

---

## 2026-03-22 每周蒸馏（近 7 天）

1. **Codex 默认模型切换前，先确认 OAuth 健康与 provider 是否正确**
   本周实际排障再次证明：把 `openai-codex/gpt-5.4` 设成默认模型之前，必须先确认 `openai-codex` provider 已正确登录，且 refresh token 没失效。否则看起来像是“模型名不对”或“配置没生效”，本质上往往是鉴权链路先坏了。

2. **Codex 登录排障要明确提醒：授权发生在 OpenClaw 的独立浏览器，不是日常 Chrome**
   用户在网页里说“我已经登录了”时，仍可能只是看着普通 Chrome，没有在真正承载 OAuth 回调的 OpenClaw 浏览器里完成操作。后续遇到 Codex 登录卡住，应该优先确认弹出的独立浏览器窗口是否被实际操作，再决定是否重启登录流程。

3. **`config.patch` 不适合做“删除旧模型配置”的清理类操作**
   本周再次暴露出一个边界：想删除已有的 Codex 相关 key 时，`config.patch` 只会合并，不会清除旧字段。凡是“删除 provider / 删除默认模型残留 / 重置一段配置”这类清理动作，默认改用直接编辑 JSON 再重启，不要指望 patch 自动收拾干净。

4. **Cron/定时技能的首要排障项应前置为“运行环境有没有工具能力”**
   Roam 整理和书签整理这类任务，本周出现的并不是脚本逻辑错误，而是任务被丢进了缺少 `exec/read` 的环境里执行。以后看到“当前运行环境未提供工具”这类报错时，应直接把排查重点放到 session / agent / tool 绑定，而不是先去改技能代码。
