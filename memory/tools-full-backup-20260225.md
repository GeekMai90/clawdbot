...
**笔记结构：**
- `00-收件箱/` - 临时存放，定期整理
- `30-运行记录/私人日记/` - 月度个人日记（格式：YYYY-MM.md）
...
**自动操作流程：**
1. 定位到 `30-运行记录/私人日记/YYYY-MM.md`（当月日记文件，不存在则创建）
2. 在文件末尾追加格式化内容：
   ```
...
VAULT="/Users/geekmai/Library/Mobile Documents/iCloud~md~obsidian/Documents/GeekMaiOB"
MONTH=$(date '+%Y-%m')
DIARY_FILE="$VAULT/30-运行记录/私人日记/$MONTH.md"
...
**数据位置（Obsidian）**: `GeekMaiOB/30-运行记录/财务记录/`
- 月账单：`YYYY-MM.md`
- 待报销：`待报销.md`
- 订阅：`订阅管理.md`
...
**触发方式**: 麦先生说「整理书签」或「整理收集箱」（AI 智能分析模式：我逐条判断分类/alias/tags，比纯脚本准确）
...
## Notion 选题记录（notion-topic）📝

**触发方式**: 麦先生说「选题 xxx」或「添加选题 xxx」

**数据库**: 内容选题库 Content Topics (`312b4958-904e-8031-9c59-000b830ad1bd`)

**工作流程**:
1. 提取想法内容
2. AI 自动拟标题（10-30字）
3. 创建 Notion 页面（状态=灵感，优先级=中）
4. 写入正文（完整想法 + 「💡 选题想法」标题）
5. 回复确认 + 页面链接

**字段**:
- 选题标题（title）- AI 生成
- 状态（status）- 灵感/调研/待写/写作中/已发布/归档
- 优先级（select）- 高/中/低
- 正文（blocks）- 完整想法
...
- Version: `2025-09-03`
- 创建页面: `POST /v1/pages`
- 写入正文: `PATCH /v1/blocks/{page_id}/children`
